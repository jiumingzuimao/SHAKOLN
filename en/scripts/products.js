...
var pathPrefix = (function() {
    ...
    if (parts.length >= 2 && (parts[1] === 'en' || parts[1] === 'ru' || parts[1] === 'ar')) {
        console.log('检测到多语言目录，路径前缀设为: ../');
        return '../';
    }
    console.log('检测到根目录，路径前缀设为: (空)');
    return '';
})();

console.log('当前使用的路径前缀:', pathPrefix);
...
// 1. Banner 轮播功能
        ...
        async function loadCarModelsData() {
            ...
            try {
                const response = await fetch(pathPrefix + 'car-models-data.json', { cache: 'force-cache' });
        ...
        function changeSeries() {
            ...
            if (brand === '') {
                seriesSelect.innerHTML = '<option value="">请选择车系</option>';
        ...
        async function searchByVIN() {
            ...
            if (!carInfo.chassis) {
                try {
                    const response = await fetch(pathPrefix + 'vin-data.json');
                    if (!response.ok) {
        ...
        async function showResults(searchType, carInfo) {
            ...
            try {
                // 加载产品数据
                const productResponse = await fetch(pathPrefix + 'product-data.json');
                const productData = await productResponse.json();
                products = productData.products || [];
                console.log('加载产品数量:', products.length);
                
                // 加载分类结构数据
                const categoryResponse = await fetch(pathPrefix + 'category-structure.json');
        ...
        async function loadHomeProducts() {
            ...
            // 优先从API获取数据
            try {
                const response = await fetch(pathPrefix + 'data/home_products.json?' + Date.now());
                console.log('产品中心响应状态:', response.status);
                if (response.ok) {
                    products = await response.json();
                }
            } catch (error) {
                console.log('加载产品数据失败:', error);
            }
        ...
        // 辅助函数：正确URL编码路径中的每个部分
        function encodePath(path) {
            if (!path) return path;
            const parts = path.split('/');
            const encodedParts = parts.map(function(part) {
        ...
        function renderHomeProducts(products) {
            ...
            const defaultImages = [
                'img/chexing/benchiCji.jpg',
                'img/chexing/BMW3xi.jpg',
                'img/chexing/benchiEji.jpg',
                'img/chexing/benchiSji.jpg',
                'img/chexing/BMW5xi.jpg'
            ];
        ...
        async function loadSelectedProducts() {
            ...
            try {
                const response = await fetch(pathPrefix + 'data/hot_products.json?' + Date.now());
                if (response.ok) {
                    const data = await response.json();
                    products = data.products || data || [];
                }
            } catch (e) {
                console.log('加载爆款数据失败:', e);
            }
