// 产品加载脚本 - 阿拉伯文版 - 从 product-data.json 和 product-images.json 加载
// 产品图片从 Gitee 仓库加载
console.log('=== Product Loading Script Started (Arabic) ===');
...
const arTranslator = {
    categories: {
        '发动机配件': 'قطع الغيار للمحرك',
        '变速箱配件': 'قطع غيار ناقل الحركة',
        '制动系统': 'نظام الفرامل',
        '转向系统': 'نظام التوجيه',
        '车身内饰': 'أجزاء داخلية',
        '车灯照明': 'الإضاءة',
        ...
        '悬挂减震系统': 'نظام التعليق',
        '冷却系统': 'نظام التبريد',
        '电器电控': 'النظام الكهربائي، الإلكترونية',
        '底盘传动系统': 'الهيكل ونظام الدفع، ناقل الحركة',
        '进气排气系统': 'نظام السحب والعادم',
        '车身外观': 'أجزاء خارجية',
        '通用密封橡胶件': 'الحشوات والمطاط العام',
        '精品系列': 'فئة المميزة'
    },
    productNames: {
        '副水壶': 'Expansion Tank',
    ...
    translateProduct: function(product) {
        ...
        if (product.stock === '充足') translatedStock = 'متوفر';
        return {
            ...product,
            name: translatedName,
...
function getProductImagePath(productCode) {
    ...
    if (images && images.length > 0) {
        return PRODUCT_IMAGE_BASE_URL + images[0];
    }
    return null;
}
...
function renderProductsCore(containerId, products, basePath, language) {
    ...
    // 翻译产品
    var translatedProducts = arTranslator.translateProducts(products);
    ...
    for (var i = 0; i < Math.min(translatedProducts.length, 5); i++) {
        ...
        if (hasProductImage(product.code)) {
            ...
            if (product.image.startsWith('http')) {
                ...
                imgSrc = PRODUCT_IMAGE_BASE_URL + product.image;
            } else {
                imgSrc = defaultSrc;
            }
        } else {
        ...
        card.href = basePath + 'ar/product-detail.html?oe=' + encodeURIComponent(product.code);
...
setTimeout(function() {
    ...
    if ((hotContainer && hotContainer.children.length === 0) || (homeContainer && homeContainer.children.length === 0)) {
        ...
        loadHomepageProducts('hotProductsList', 'homeProductsList', 'ar');
    }
}, 2000);

// 自动启动：阿拉伯文版
startLoadingProducts('ar');

console.log('=== Product Loading Script Initialization Complete (Arabic) ===');
